...
class DiceRollGame: # Class for the Dice Roll Game
    ...
    def roll_dice(self): # Function to roll the dice
        dice_result = random.randint(1, 6)
        self.result_label.config(text=f"Rolled: {dice_result}")
        self.dice_label.config(image=self.dice_images[dice_result - 1])

    def go_back(self): # Function to go back to the main menu
        self.root.destroy()
        subprocess.run(["python", "menu.py"])

    def center_window(self, width=400, height=400): # Center the window on the screen
